...
from common.ui_components import default_log_callback, summarize_results
...
if st.button("⚠️ Streamlitキャッシュ全クリア", key="system2_clear_cache"):
    st.cache_data.clear()
...
def main_process(use_auto, capital, symbols_input):
    ...
    def log_callback(i, total, start_time):
        bt_log.text(
            default_log_callback(i, total, start_time, prefix="💹 バックテスト")
        )
    ...
    if not summary_df.empty:
        ...
        signal_path = os.path.join(
            signal_dir,
            f"system2_signals_{today_str}_{
                int(capital)}.csv",
        )
...
use_auto = st.checkbox(
    "自動ティッカー取得（全銘柄）", value=True, key="system2_auto_main"
)
capital = st.number_input(
    "総資金（USD）", min_value=1000, value=1000, step=100, key="system2_capital_main"
)
symbols_input = None
if not use_auto:
...
def run_tab():
    ...
    use_auto = st.checkbox(
        "自動ティッカー取得（全銘柄）", value=True, key="system2_auto_tab"
    )
    capital = st.number_input(
        "総資金（USD）", min_value=1000, value=1000, step=100, key="system2_capital_tab"
    )
    symbols_input = None
    if not use_auto:
        symbols_input = st.text_input(
