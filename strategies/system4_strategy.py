...
class System4Strategy:
    ...
    # インジケーター計算
    # ===============================
    def prepare_data(self, raw_data_dict, progress_callback=None, log_callback=None, skip_callback=None, batch_size=50):
        result_dict = {}
        ...
        for sym, df in raw_data_dict.items():
            ...
            # ---- スキップ件数は専用のコールバックに流す ----
            if processed == total and skipped > 0:
                if skip_callback:
                    skip_callback(f"⚠️ データ不足/計算失敗でスキップ: {skipped} 件")
                elif log_callback:  # フォールバック
