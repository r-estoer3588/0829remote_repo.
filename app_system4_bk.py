...
st.title("システム4：ロング・トレンド・ロー・ボラティリティ（複数銘柄）")
...
def main_process(use_auto, capital, symbols_input):
    ...
    ind_log = st.empty()  # 進捗ログ用
    ind_skip_log = st.empty()  # ⚠️ スキップ件数用 ← 追加
    ...
    for i, (sym, df) in enumerate(prepared_dict.items(), 1):
        ...
        df.to_csv(path)
        progress_bar.progress(i / total)
        status_text.text(f"💾 System4キャッシュ保存中: {i}/{total} 件 完了")
    status_text.text(f"💾 System4キャッシュ保存完了 ({total} 件)")
    progress_bar.empty()
    st.success("🔚 バックテスト終了")
...
use_auto = st.checkbox(
    "自動ティッカー取得（全銘柄）", value=True, key="system4_auto_main"
)
capital = st.number_input(
    "総資金（USD）", min_value=1000, value=1000, step=100, key="system4_capital_main"
)
symbols_input = None
if not use_auto:
...
def run_tab():
    ...
    use_auto = st.checkbox(
        "自動ティッカー取得（全銘柄）", value=True, key="system4_auto_tab"
    )
    capital = st.number_input(
        "総資金（USD）", min_value=1000, value=1000, step=100, key="system4_capital_tab"
    )
    symbols_input = None
    if not use_auto:
        symbols_input = st.text_input(
